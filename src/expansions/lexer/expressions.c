...
ssize_t	add_simple_quotes_expression(char *arg, t_list **expansions)
{
	...
	if (next_quote == NULL)
		return (-1);
	if (add_expansion_node(NULL, SIMPLE_QUOTE, expansions) < 0)
		return (-1);
	word = ft_substr(arg, 0, next_quote - arg);
	if (word == NULL)
		return (-1);
	if (add_expansion_node(word, WORD, expansions) < 0)
		return (free(word), -1);
	if (add_expansion_node(NULL, SIMPLE_QUOTE, expansions) < 0)
		return (-1);
...
ssize_t	add_dollar_expression(char *arg, t_list **expansions)
{
	...
	if (add_expansion_node(NULL, DOLLAR, expansions) < 0)
		return (-1);
...
ssize_t	add_double_quotes_expression(char *arg, t_list **expansions)
{
	...
	len = 0;
	if (add_expansion_node(NULL, DOUBLE_QUOTE, expansions) < 0)
		return (-1);
	arg++;
	while (*arg != '"')
	{
